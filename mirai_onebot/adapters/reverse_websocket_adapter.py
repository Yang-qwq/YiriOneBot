...
class ReverseWebsocketAdapter(Adapter):
    ...
    async def handler(self, websocket: websockets.WebSocketServerProtocol, path: str):
        # 检测OneBot标准版本
        protocol_str = websocket.request_headers.get("Sec-WebSocket-Protocol")
        ...
        if (
            websocket.request_headers.get("Authorization", "").endswith(
                self.access_token
            )
            or query.get("access_token", "") == self.access_token
        ):
            ...
            logger.warning(
                "一个OneBot实现在建立的过程中未正确传递 access_token，请检查设置。"
            )
            await websocket.close(
                websockets.frames.CloseCode.NORMAL_CLOSURE, "Unauthorized"
            )
    ...
    async def recv(self, websocket: websockets.WebSocketServerProtocol):
        ...
        try:
            ...
            if not isinstance(data, dict):
                logger.warning(
                    f"OneBot 实现 {websocket.remote_address[0]} 发送了非字典数据 {data}。"
                )
                return None
        ...
        # 解析是事件还是响应
        if (
            data.get("detail_type", None) is None and data.get("sub_type", None) is None
        ):  # 是响应或其他
    ...
    async def _call_api(
        ...
        await self.ws_connections[0].send(
